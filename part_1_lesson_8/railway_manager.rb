...
class RailwayManager
  ...
  def create_wagon
    ...
    begin
      ...
      when 1
        ...
        @wagons_list << PassengerWagon.new(number, total_seats_number)
      when 2
        puts "Введите объем вагона"
        total_volume = gets.chomp.to_f
        @wagons_list << CargoWagon.new(number, total_volume)
      end
  ...
  def view_trains_list_in_station
    ...
    if station.trains_list.empty?
      ...
      station.each_train { |train| puts "Номер: #{train.number}, тип: #{train.type}, число вагонов: #{train.wagons.count}" }
    end
  end

  def view_wagons_list_in_train
    puts "Выберите и введите номер поезда для вывода списка вагонов из имеющихся вариантов: #{extract_numbers(@trains_list).join(", ")}"
    train_name = gets.chomp
    train = @trains_list.find { |train| train.number == train_name }
    ...
    if train.wagons.empty?
      puts "У данного поезда отсутсвуют вагоны!"
      return
    end
    ...
    if train.type == 'пассажирский'
      ...
      train.each_wagon { |wagon| puts "#{wagon.number}, #{wagon.type}, #{wagon.taken_volume}, #{wagon.available_volume}" }
    end
  end

  def take_place
    puts "Выберите и введите номер поезда из имеющихся вариантов: #{extract_numbers(@trains_list).join(", ")}"
    train_name = gets.chomp
    train = @trains_list.find { |train| train.number == train_name }
    ...
    puts "Выберите и ведите номер вагона из имеющихся вариантов: #{extract_numbers(train.wagons).join(", ") }"
    wagon_number = gets.chomp
    wagon = train.wagons.find { |wagon| wagon.number == wagon_number }
