...
class RailwayManager
  ...
  def seed
    # Создаем станции
    @stations_list << Station.new("Minsk")
    @stations_list << Station.new("Mogilev")
    @stations_list << Station.new("Orsha")

    # Создаем поезда
    @trains_list << PassengerTrain.new("pass1")
    @trains_list << PassengerTrain.new("pass2")
    @trains_list << CargoTrain.new("cargo1")

    # Создаем маршруты
    @routes_list << Route.new(@stations_list[0], @stations_list[1], "route1")
    @routes_list << Route.new(@stations_list[2], @stations_list[1], "route2")

    # Добавляем маршруты к поездам
    @trains_list[0].add_route(@routes_list[0])
    @trains_list[0].add_route(@routes_list[1])

    # Создаем вагоны
    @wagons_list << CargoWagon.new("1")
    @wagons_list << PassengerWagon.new("2")
  end
